...
class MinistryDaily extends Controller
{
  ...
  public function store(Request $request)
  {
    ...
    //  dd($userID);
      ...
      if($final==1){
       ...
       // dd($monthID);
        if($monthID >= 1){
          $this->genUpdateMonth($m, $monthID);  
        } else {
          // no record yet then create one
  ...
  function service_report(Request $request){
    ...
    $data = ['LoggedUserInfo'=>User::where('userID','=', $userID)->first()];
        ...
        return view('auth.servicereport', $data, compact('title','events','monthrpt'));

  }



  function update_monthly(Request $request, $id, $month ){
    // dd($request);
    $M = sprintf("%02d", $month);
